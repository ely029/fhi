<div class="section__container">
    ...
    <a href="{{ url('treatment-outcomes?status=Referred to National Chair') }}">
            <li class="tabs__item {{ request('status') == 'Referred to National Chair' ? 'tabs__item--current' : ''}}">Referred cases({{ $referredCases->count() }})</li>
        </a>
        <a href="{{ url('treatment-outcomes?status=Referred back to Regional Chair') }}">
            <li class="tabs__item {{ request('status') == 'Referred back to Regional Chair' ? 'tabs__item--current' : ''}}">Completed({{ $completed->count() }})</li>
        </a>
    <a href="{{ url('treatment-outcomes?treatmentOutcomeTabs=all_cases_ntb_chair') }}">
            <li class="tabs__item {{ request('treatmentOutcomeTabs') == 'all_cases_ntb_chair' ? 'tabs__item--current' : ''}}">All cases({{ $allCases->count() }})</li>
        </a>
    </ul>
